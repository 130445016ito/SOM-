	...
	
{
	...
	*/
		...
		//時系列データのサンプル数
		numSample=500;//既存値＝40


	//コンストラクタ，ここでアロケートする(1/19)++++++++++++++++++++//
		// Allocate
		accArrDlg=new double*[6];
		for(ui i=0; i<6; i++) {
			accArrDlg[i] = new double[numSample];
		}

		curAccArrDlg=new double[6];
		//for(ui i=0; i<3; i++) {
...
//	}
	...
	//}
	 ...
	 if(packet.smid==4){
		...
		myVar.curAccArrDlg[2] = msg.acc[2];
	...
	//}
	 ...
	 if(packet.smid==5){
		myVar.curAccArrDlg[3] = msg.acc[0];
		myVar.curAccArrDlg[4] = msg.acc[1];
		myVar.curAccArrDlg[5] = msg.acc[2];
		//myVar.curAccArrDlg[3] = msg.ang[0];
		//myVar.curAccArrDlg[4] = msg.ang[1];
		//myVar.curAccArrDlg[5] = msg.ang[2];
	//}
